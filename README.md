...
> mpk3s는 Multipass 기반의 K3s 클러스터를 관리하기 위한 통합 플랫폼입니다. 강력한 CLI 도구와 직관적인 Web UI를 통해 Kubernetes 클러스터를 광속으로 구축하고 관리할 수 있습니다.
...
- **통합 CLI**: GUI 없이 터미널에서도 모든 기능을 `mpk3s` 명령어 하나로 제어할 수 있습니다.
...
### 2. 클러스터 생성 및 관리
```bash
mpk3s generate    # 대화형 클러스터 생성
mpk3s list        # 모든 클러스터 목록 확인
mpk3s add         # 워커 노드 추가
mpk3s delcluster  # 클러스터 삭제
```
